    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('Libro')) ]) }}</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __(\Illuminate\Support\Str::plural('Libro')) }}</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('Libro')->create && hasPermission(getRouteName().'.libro.create', 1, 1))
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.libro.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('Libro') ]) }}</a>
                        </div>
                        @endif
                        @if(getCrudConfig('Libro')->searchable())
                        <div class="col-md-4">
                ...
                <table class="table table-hover table-striped">
                    <thead>
                        <tr>
                            ...
                            @if(getCrudConfig('Libro')->delete or getCrudConfig('Libro')->update)
                    ...
                    <tbody>
                        @foreach($libros as $libro)
                            @livewire('admin.libro.single', [$libro], key($libro->id))
                        @endforeach
                    </tbody>
                </table>
            </div>
