    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('Enciclopedia')) ]) }}</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __(\Illuminate\Support\Str::plural('Enciclopedia')) }}</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('Enciclopedia')->create && hasPermission(getRouteName().'.enciclopedia.create', 1, 1))
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.enciclopedia.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('Enciclopedia') ]) }}</a>
                        </div>
                        @endif
                        @if(getCrudConfig('Enciclopedia')->searchable())
                        <div class="col-md-4">
                ...
                <table class="table table-hover table-striped">
                    <thead>
                        <tr>
                            ...
                            @if(getCrudConfig('Enciclopedia')->delete or getCrudConfig('Enciclopedia')->update)
                                <th scope="col">{{ __('Action') }}</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($enciclopedias as $enciclopedia)
                            @livewire('admin.enciclopedia.single', [$enciclopedia], key($enciclopedia->id))
                        @endforeach
                    </tbody>
                </table>
            </div>
            <div class="m-auto pt-3 pr-3">
                {{ $enciclopedias->appends(request()->query())->links() }}
            </div>
