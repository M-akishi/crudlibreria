<tr x-data="{ modalIsOpen : false }">
    <td class="">{{ $region->des_region }}</td>
    
    @if(getCrudConfig('Region')->delete or getCrudConfig('Region')->update)
        <td>

            @if(getCrudConfig('Region')->update && hasPermission(getRouteName().'.region.update', 1, 0, $region))
                <a href="@route(getRouteName().'.region.update', $region->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('Region')->delete && hasPermission(getRouteName().'.region.delete', 1, 0, $region))
